document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyFilters() {
        ...
        currentFilteredPapers = result;
        displayPapers(currentFilteredPapers);
    }
    ...
    // --- Export Functions ---
    function getPapersForExport() {
        return currentFilteredPapers;
    }

    function getCategoriesForPapers(papers) {
    ...
    function getDateRangeForPapers(papers) {
        const dates = papers
    ...
    function showNoExportablePapersMessage() {
        closeExportMenu();
        showStatus('No papers match the current filters.', false);
    }

    function exportToJson() {
        try {
            const papersToExport = getPapersForExport();
            if (papersToExport.length === 0) {
                showNoExportablePapersMessage();
                return;
            }

            const exportData = {
                exportDate: new Date().toISOString(),
                totalPapers: papersToExport.length,
                totalAvailablePapers: allPapers.length,
                dateRange: getDateRangeForPapers(papersToExport),
                categories: getCategoriesForPapers(papersToExport),
                papers: papersToExport.map(paper => ({
                    ...paper,
    ...
    function exportToMarkdown() {
        try {
            ...
            const exportCategories = getCategoriesForPapers(papersToExport);
            const exportDateRange = getDateRangeForPapers(papersToExport);
            const today = new Date().toLocaleDateString();
            let markdown = `# foodAI - Living Review on Generative AI in AgriFood\n\n`;
            markdown += `**Generated:** ${today}  \n`;
            markdown += `**Total Papers:** ${papersToExport.length}  \n`;
            markdown += `**Total Available Papers:** ${allPapers.length}  \n`;
            markdown += `**Date Range:** ${exportDateRange.minDate || 'N/A'} - ${exportDateRange.maxDate || 'N/A'}  \n\n`;

            markdown += `## Categories\n\n`;
            exportCategories.forEach(category => {
                const count = papersToExport.filter(p => p.categories?.includes(category)).length;
                markdown += `- **${category}**: ${count} papers\n`;
            });
            markdown += `\n`;

            exportCategories.forEach(category => {
                const categoryPapers = papersToExport.filter(p => p.categories?.includes(category));
